...
def Loc_Trung_binh_so_hoc(img, ksize):
    ...
    h=(ksize -1) // 2 # tăng giá trị pixcel lề ảnh
    padded_img = np.pad(img, (h, h), mode='reflect') # thêm vào lề giá trị(hxh)-> (y,x)
    for i in range(m):
        for j in range(n):
            vung_anh_kich_thuoc_k = padded_img[i:i+ksize,j:j+ksize]
            img_ket_qua_anh_loc[i,j] = np.mean(vung_anh_kich_thuoc_k) # Tính trung bình của vùng ảnh kích thước
...
if __name__ == "__main__":
    ...
    fig = plt.figure(figsize=(9, 9))     # Thiết lập vùng (cửa sổ) vẽ
